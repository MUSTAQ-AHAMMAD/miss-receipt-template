...
class OracleFusionIntegration:
    ...
    def _load_payment_file(
        ...
        for _, row in pf.iterrows():
            ...
            # Skip payments with no Order Ref or zero amount
            if not inv or amount == 0:
                continue

            # CRITICAL FIX: Only load payments that match AR Invoice entries
            # Unmatched payments should be ignored to prevent generating receipts for non-existent invoices
            ...
            # Add payment data only for matched invoices
        ...
        if unmatched:
            ...
            print(f"    → These unmatched payments are IGNORED and will NOT generate receipts")
        print(f"  ✓ Payment file loaded: {len(result):,} matched invoice references (ignored {unmatched:,} unmatched entries)")
        return (result, dates)
        ...
        if payment_file_path and Path(payment_file_path).exists():
            ...
            if payment_result is not None:
                ...
                for inv_ref, methods in payment_data.items():
                    # Since _load_payment_file now only returns matched invoices,
                    # all entries here should already be in self.invoice_payments
                    if inv_ref in self.invoice_payments:
                        # Clear Cash default and replace with real payment methods
