...
- Quantity sign matches amount sign (returns handled correctly)
- Unit Selling Price = amount / quantity
...
AR_STATIC: Dict[str, str] = {
    ...
    # Unit of Measure Code removed — read from line items file instead
...
LINE_ITEMS_COL_MAP = {
    ...
    # ── UoM: read from column E "Order Lines/Base UoM" ──────────────
    "Unit of Measure": [
...
class TxnNumberGenerator:
    ...
    def get_normal(self, store_name: str, sale_date) -> str:
        ...
        if key not in self._normal_cache:
            ...
            self._normal_cache[key] = f"BLK-{alphanumeric_id}"
            self._normal_seq += 1
    ...
    def get_bnpl(self, store_name: str, sale_date, customer_type: str) -> str:
        ...
        if key not in self._bnpl_cache:
            ...
            self._bnpl_cache[key] = f"BLK-{alphanumeric_id}"
            self._bnpl_seq += 1
...
class OracleFusionIntegration:
    ...
    def __init__(
        ...
        self._seg1_prefix       = _generate_run_prefix(8)   # e.g. "A3F2B1C9"
        self._seg2_prefix       = _generate_run_prefix(8)   # e.g. "X7K9M2Q4"
    ...
    def load_data(
        ...
        vl.kv("  NORMAL  first number",        f"BLK-{alphanumeric_start}")
        alphanumeric_start_bnpl = TxnNumberGenerator._to_alphanumeric(self.start_seq, 4)
        vl.kv("  TABBY   first number",        f"BLK-{alphanumeric_start_bnpl}")
        vl.kv("  TAMARA  first number",        f"BLK-{alphanumeric_start_bnpl}")
    ...
    def _normalise_line_items(self):
        ...
        # Drop rows where Order Ref is blank after cleaning
        self.line_items = self.line_items[
            self.line_items["Order Ref"] != ""
        ].reset_index(drop=True)

        # Build O(1) index: Order Ref → list of row positions
    ...
    def generate_ar_invoices(self) -> pd.DataFrame:
        ...
        for inv in invoices:
            ...
            for pos in row_positions:
                ...
                # ── Unit of Measure: read from line items, write to Name only ──
                ...
                # ── Segment values: random prefix + sequential number ──
                ...
                # Unit of Measure Code = blank; Name = value from line items
                ...
                # If barcode is empty OR it is a discount line → treat as
                # discount item: blank Inventory Item Number, set Memo Line.
        ...
        vl.kv("Max Transaction Number used",         f"BLK-{alphanumeric_max}")
    ...
    def save_ar(self, df: pd.DataFrame):
        ...
        # ── Filename: org name + date e.g. AR_Invoice_ALQURASHI_KSA_05_31_May2026.csv ──
        run_date  = datetime.now()
        date_part = run_date.strftime("%m_%d_%b%Y")                    # e.g. 05_31_May2026
        org_name  = safe_filename(AR_STATIC.get("Comments", "ORG"))    # e.g. ALQURASHI_KSA
...
def main():
    ...
    # Auto-detect which xlsx is line_items vs payments
    ...
    # Fallback to explicit names if auto-detect failed
    ...
    INPUT = {
        "line_items":      "HAYAT Sale line 5 to 31 March.xlsx",
        "payments":        "HAYAT payment line 5 to 31 March.xlsx",
    ...
    START_TXN_SEQUENCE  = 200   # ← update from report "Next run START_TXN_SEQUENCE ="
